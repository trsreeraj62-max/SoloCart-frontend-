...
async function fetchOrderDetail(id) {
    try {
        ...
        if (window.showToast) window.showToast('Signal lost while retrieving order', 'error');
    }
}
...
function renderDetails(order) {
    ...
    if (document.getElementById('display-order-id')) document.getElementById('display-order-id').innerText = `#ORD-${order.order_number || order.id}`;
    if (document.getElementById('display-order-date')) document.getElementById('display-order-date').innerText = new Date(order.created_at).toLocaleString();
    if (document.getElementById('total-amount')) document.getElementById('total-amount').innerText = `₹${Number(order.total_amount || 0).toLocaleString()}`;
    if (document.getElementById('payment-method')) document.getElementById('payment-method').innerText = order.payment_method || 'N/A';
    if (document.getElementById('order-status-text')) document.getElementById('order-status-text').innerText = `Current status: ${order.status || 'PENDING'}`;
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            ...
            const invoiceUrl = `${CONFIG.API_BASE_URL}/orders/${order.id}/invoice?token=${getAuthToken()}`.replace(/^http:/, 'https:');
